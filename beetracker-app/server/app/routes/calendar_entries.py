...
def get_group_period_entries(
    ...
    cal_entries = []
    for user in users:
        entries = db.query(CalendarEntries).filter(CalendarEntries.__table__.c.user_id == user.Id).all()
...
def create_group_period_entry(
    ...
    existing_entry = db.query(CalendarEntries).filter(CalendarEntries.__table__.c.id == calendar_entries_schema.id).first()
    ...
    if existing_entry:
        ...
        new_entry.id = calendar_entries_schema.id or uuid.uuid4()
        db.add(new_entry)
...
def delete_group_period_entry(
    ...
    entry = db.query(CalendarEntries).filter(CalendarEntries.__table__.c.id == calendar_entry_id).first()
    if not entry:
