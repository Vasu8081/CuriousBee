        ...
        with open(model_file, "w") as f:
            ...
            for col_name, col_type, modifiers in columns:
                ...
                if fk_target:
                    ...
                    circular = (table, tgt) in circular_deps or (tgt, table) in circular_deps
                    col_def += f", ForeignKey(\"{fk_target}\"" + (", use_alter=True" if circular else "") + ")"
                if is_pk:
            ...
            for fk in fks.get(table, []):
                ...
                fld_name = fk["src_col"][1:-3]
                uselist = "uselist=False" if rel_type in ["many_to_one", "one_to_one"] else "uselist=True"
                back_pop = f"back_populates='{table}'"
                f.write(f"    {fld_name} = relationship('{tgt_class}', {uselist}, {back_pop})\n")

            # Reverse Relationships
            for rfk in reverse_fks.get(table, []):
                rel_type = rfk["type"]
                src_class = to_pascal(rfk["src_table"])
                fld_name = rfk["src_table"].lower() if rel_type in ["many_to_many", "many_to_one"] else rfk["src_col"][1:-3]
                uselist = "uselist=True" if rel_type in ["many_to_many", "one_to_many"] else "uselist=False"
                back_pop = f"back_populates='{fld_name}'"
                f.write(f"    {fld_name} = relationship('{src_class}', {uselist}, {back_pop})\n")

            # Helper methods
            f.write("\n")
            for col_name, col_type, _ in columns:
                method = to_camel(col_name.lstrip("_"))
                py_type = PY_TYPES.get(col_type.upper(), "str")
                f.write(f"    def {method}(self, value: {py_type} = None) -> {py_type}:\n")
